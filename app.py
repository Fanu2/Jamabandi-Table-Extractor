...
def extract_tables_from_pdf(file):
    ...
    if len(tables) > 1:
        ...
        for t in tables[1:]:
            # Force same columns. Even if broken piece has no header, use base_cols
            t = pd.DataFrame(t.values, columns=base_cols)
...
if uploaded_file:
    ...
    if not tables:
        ...
        st.success(f"Extracted and merged into {len(tables)} table(s).")

        df = tables[0]
        st.subheader("Preview of Merged Table")
        st.dataframe(df)
